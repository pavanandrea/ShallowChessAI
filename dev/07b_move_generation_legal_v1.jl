...
function pseudolegalmoves(currentbitboard,player=1)
    ...
    childrenboards = Vector{Vector{Float32}}(undef,0);
    if player==1
        #white pawns
        for i=1+8:64-8
            #one square forward
...
function legalmoves(currentbitboard,player=1)
    ...
    #childrenboards = [];
    moves = Vector{Vector{Int}}(undef,0);
    childrenboards = Matrix{Float32}(undef,(783,0));
    for i=1:lastindex(candidatemoves)
        #=if !ischecked(candidateboards[i],player)
            push!(moves,candidatemoves[i]);
            push!(childrenboards,candidateboards[i]);
        end=#
