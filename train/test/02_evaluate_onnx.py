...
def compute_target_score(cp: float | None, mate: int | None) -> float | None:
    ...
    if cp is not None:
        return 2.0 * math.tanh(np.clip(cp, -1500.0, 1500.0) / 364.0)
    return None

def score_to_cp(score: float) -> float:
    s = float(np.clip(score / 2.0, -0.999_999, 0.999_999))
    return 364.0 * float(np.arctanh(s))
...
def _plot_errors(cp_err:np.ndarray, out_path:Path, title:str):
    ...
    if len(cp_err) > 0:
        ...
        if len(display) > 0:
            ...
            #ax.axvline(np.median(display), color="darkorange", linestyle=":",
            #           linewidth=1.2, label=f"P50 = {np.median(display):.1f} cp")
            #ax.axvline(np.percentile(display, 95), color="navy", linestyle=":",
            #           linewidth=1.2, label=f"P95 = {np.percentile(display, 95):.1f} cp")
